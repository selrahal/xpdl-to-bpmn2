...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xsd="http://www.w3.org/2001/XMLSchema" xmlns:xpdl2="http://www.wfmc.org/2009/XPDL2.2" xmlns:drools="http://www.jboss.org/drools" xmlns:bpmn2="http://www.omg.org/spec/BPMN/20100524/MODEL" xmlns:color="http://www.omg.org/spec/BPMN/non-normative/color" xmlns:bpmndi="http://www.omg.org/spec/BPMN/20100524/DI" version="1.0">
  ...
  <xsl:template match="xpdl2:Activity/xpdl2:Implementation/xpdl2:Task/xpdl2:TaskScript">
  ...
  <xsl:template match="xpdl2:Activity[not(xpdl2:Implementation) and not(xpdl2:Event) and not(xpdl2:Route)]">
    <bpmn2:task drools:selectable="true" color:background-color="#fafad2" color:border-color="#000000" color:color="#000000" >
      <xsl:variable name="origId" select="@Id"/>
      <xsl:variable name="nodeId" select="translate($origId,':','_')"/>
      <xsl:variable name="origName" select="@Name"/>
      <xsl:variable name="nodeName" select="translate($origName,':','_')"/>
  ...
  <xsl:template match="xpdl2:Activity/xpdl2:Implementation/xpdl2:Task/xpdl2:TaskService">
    <bpmn2:serviceTask drools:selectable="true" color:background-color="#fafad2" color:border-color="#000000" color:color="#000000" >
      <xsl:variable name="origId" select="../../../@Id"/>
      <xsl:variable name="nodeId" select="translate($origId,':','_')"/>
      <xsl:variable name="origName" select="../../../@Name"/>
      <xsl:variable name="nodeName" select="translate($origName,':','_')"/>
