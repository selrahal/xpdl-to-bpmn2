...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xsd="http://www.w3.org/2001/XMLSchema" xmlns:xpdl="http://www.wfmc.org/2004/XPDL2.0alpha" xmlns:drools="http://www.jboss.org/drools" xmlns:bpmn2="http://www.omg.org/spec/BPMN/20100524/MODEL" xmlns:color="http://www.omg.org/spec/BPMN/non-normative/color" xmlns:bpmndi="http://www.omg.org/spec/BPMN/20100524/DI" xmlns:xpdExt="http://www.tibco.com/XPD/xpdExtension1.0.0" version="1.0">
  ...
  <!-- Plain End Event -->
  <xsl:template match="xpdl:Event/xpdl:EndEvent">
  ...
  <!-- Terminate End Event -->
  <xsl:template match="xpdl:Event/xpdl:EndEvent[@Result='Terminate']">
    <xsl:variable name="origId" select="../../@Id"/>
    <xsl:variable name="nodeId" select="translate($origId,':','_')"/>
    <xsl:variable name="origName" select="../../@Name"/>
    <xsl:variable name="nodeName" select="translate($origName,':','_')"/>
  ...
  <!-- Signal End Event -->
  <xsl:template match="xpdl:Event/xpdl:EndEvent[@Result='Signal']">
    <xsl:variable name="origId" select="../../@Id"/>
    <xsl:variable name="nodeId" select="translate($origId,':','_')"/>
    <xsl:variable name="origName" select="../../@Name"/>
    <xsl:variable name="nodeName" select="translate($origName,':','_')"/>
    <bpmn2:endEvent drools:bgcolor="#ff6347" drools:selectable="true">
      ...
      <bpmn2:signalEventDefinition>
        ...
        <xsl:attribute name="signalRef">
          <xsl:value-of select="xpdl:TriggerResultSignal/@Name"/>
        </xsl:attribute>
      </bpmn2:signalEventDefinition>
    </bpmn2:endEvent>
  </xsl:template>
